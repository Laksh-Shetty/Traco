  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      return await db.$transaction(async (tx) => {
        // 1. Fetch the template transaction
        ...
        await tx.transaction.create({
          data: {
            ...
            date: new Date(), // Date of actual creation
            category: template.category,
            userId: template.userId,
            accountId: template.accountId,
            isRecurring: false, // The new one isn't a template itself
          },
        });

        // 4. Update the template with the new "next date"
        ...
        // 5. Update account balance (since it's a new transaction)
